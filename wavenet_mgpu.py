...
def build_model(fragment_length, nb_filters, nb_output_bins, dilation_depth, nb_stacks, use_skip_connections,
    ...
    def residual_block(x):
        ...
        tanh_out = CausalDilatedConv1D(nb_filters, 2, atrous_rate=2 ** i, border_mode='valid', causal=True,
                                             bias=use_bias,
                                             name='dilated_conv_%d_tanh_s%d' % (2 ** i, s), activation='tanh',
                                             W_regularizer=l2(res_l2))(x)
        sigm_out = CausalDilatedConv1D(nb_filters, 2, atrous_rate=2 ** i, border_mode='valid', causal=True,
                                             bias=use_bias,
    ...
    input_shape = Input(shape=(fragment_length, nb_output_bins), name='input_part')
    out = input_shape
    skip_connections = []
    out = CausalDilatedConv1D(nb_filters, 2, atrous_rate=1, border_mode='valid', causal=True,
    ...
    out = layers.Conv1D(nb_output_bins, 1, padding='same',
